...
if ($saveOrder) {
	...
	$saveOrderingUrl = 'index.php?option=com_unavailability&task=systems.saveOrderAjax&tmpl=component';
	// unavailabilityList id matches the unavailabilityList table above
	JHtml::_('sortablelist.sortable', 'systemList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
?>

<form action="<?php echo JRoute::_('index.php?option=com_unavailability&view=systems');?>" method="post" name="adminForm" id="adminForm">
		...
		<?php } else { // @see layout helper searchtools ?>
			<table class="table table-striped" id="systemList">
				<thead>
					<tr>
						...
						<th width="10%" class="nowrap hidden-phone">
							<?php echo JHtml::_('searchtools.sort', 'COM_UNAVAILABILITY_FIELD_URL_LABEL', 'a.url', $listDirn, $listOrder); ?>
						</th>																							
				...
				<?php foreach ($this->items as $i => $item) { 
					$item->edit_link = JRoute::_('index.php?option=com_unavailability&task=system.edit&id='.(int)$item->id);
				...
				?>
					<tr class="row<?php echo $i % 2;?>" sortable-group-id="<?php echo $item->catid; ?>">
						...
						<td class="center">
							<div class="btn-group">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'systems.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
								<?php // Create dropdown items and render the dropdown list.
								if ($canChange) {
									JHtml::_('actionsdropdown.' . ((int) $item->state === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'systems');
									JHtml::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'systems');
						...
						<td>
							<div class="pull-left">
								<?php if ($item->checked_out) : ?>
									<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'systems.', $canCheckin); ?>
								<?php endif; ?>
								<?php if ($canEdit) { ?>
