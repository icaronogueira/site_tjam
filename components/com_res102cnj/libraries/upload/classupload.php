                ...
                if ($this->mime_fileinfo) {
                    ...
                    if (function_exists('finfo_open')) {
                        ...
                        if (is_resource($f)) {
                            ...
                            if (preg_match("/^([\.-word]+)\/([\.-word]+)(.*)$/i", $this->file_src_mime)) {
                                $this->file_src_mime = preg_replace("/^([\.-word]+)\/([\.-word]+)(.*)$/i", '$1/$2', $this->file_src_mime);
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->mime_check && empty($this->file_src_mime)) {
                ...
                if (!$allowed) {
                    $this->processed = false;
                    $this->error = $this->translate('incorrect_file').'. Arquivo não Permitido: '.$this->file_src_mime;
                } else {
