...
abstract class JHtmlDocumentos {
	...
	public static function links(&$items) {
		$img_url = JURI::root().'/components/com_extrajudicial/media/img/file.png';;
		...
		for($i = 0; $i < $nItems; $i++) { // para cada item...
			...
			if(property_exists($item, 'anexas')) { 
				...
				for($j = 0; $j < $nAnx; $j++) {  // para cada anexo de cada item...
					...
					switch ($anx->documento->origem) {
						case 1 : // link de download de anexo
							$anx->lnk_doc =
								'<a href="' .
									'index.php?option=com_extrajudicial&view=documento&task=download' .
