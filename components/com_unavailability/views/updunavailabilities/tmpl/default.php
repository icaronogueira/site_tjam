...
if ($saveOrder) {
	...
	$saveOrderingUrl = 'index.php?option=com_unavailability&task=updunavailabilities.saveOrderAjax&tmpl=component';
	// unavailabilityList id matches the unavailabilityList table above
	JHtml::_('sortablelist.sortable', 'updunavailabilityList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
?>

<form action="<?php echo JRoute::_('index.php?option=com_unavailability&view=updunavailabilities');?>" class="form-inline" method="post" name="adminForm" id="adminForm">
	<div>
		<?php
		...
		<?php } else {  ?>
			<table class="table table-striped" id="updunavailabilityList">
			<thead>
				<tr>
					<th width="1%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('searchtools.sort', '', 'a.ordering', $listDirn, $listOrder, null, 'asc', 'COM_UNAVAILABILITY_FIELD_ORDERING_LABEL', 'icon-menu-2'); ?>
					</th>
					<th width="1%" class="center">
						<?php echo JHtml::_('grid.checkall'); ?>
					</th>
					<th width="1%" style="min-width:55px" class="nowrap center">
						<?php echo JHtml::_('searchtools.sort', 'COM_UNAVAILABILITY_FIELD_STATUS_LABEL', 'a.state', $listDirn, $listOrder); ?>
					</th>						
					<th class="nowrap">
					...
					<th width="1%" class="nowrap center">
						<?php echo JHtml::_('searchtools.sort', 'COM_UNAVAILABILITY_FIELD_ID_LABEL', 'a.id', $listDirn, $listOrder); ?>
					</th>					
				</tr>
			</thead>
			<tfoot>
			...
			<?php foreach ($this->items as $i => $item) { 
				$item->edit_link = JRoute::_('index.php?option=com_unavailability&task=updunavailability.edit&id='.(int)$item->id);
			...
			?>
				<tr class="row<?php echo $i % 2;?>" sortable-group-id="<?php echo $item->catid; ?>">
					...
					<td class="center">
						<div class="btn-group">
							<?php echo JHtml::_('jgrid.published', $item->state, $i, 'updunavailabilities.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
							<?php // Create dropdown items and render the dropdown list.
							if ($canChange) {
								JHtml::_('actionsdropdown.' . ((int) $item->state === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'updunavailabilities');
								JHtml::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'updunavailabilities');
					...
					<td>
						<div class="pull-left">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'updunavailabilities.', $canCheckin); ?>
							<?php endif; ?>
							<?php if ($canEdit) { ?>
		...
		<div class="btn-toolbar">
			<div class="btn-group">
				<button type="button" class="btn btn-primary" onclick="Joomla.submitbutton('updunavailability.add')">
